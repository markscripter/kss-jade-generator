...
kssJadeGenerator.init = function(config) {
  ...
  this.template = this.Jade.compile(this.config.template, {
    pretty: true,
    filename: path.join(this.config.templatePath, 'index.*'),
  });
};
...
kssJadeGenerator.generate = function(styleguide) {
  ...
  Array.isArray(styleguide.data.files) ?
    console.log(styleguide.data.files.map((file) => ' - ' + file).join('\n')) :
    1;

  // Throw an error if no KSS sections are found in the source files.
  if (sectionCount === 0) throw new Error('No KSS documentation discovered in source files.');
  ...
  for (i = 0; i < sectionCount; i += 1) {
    ...
    if (sections[i].markup()) {
      ...
      if (partial.markup.match(/^[^\n]+\.(jade|html)$/)) {
        ...
        if (!files.length) partial.markup += ' NOT FOUND!';
...
kssJadeGenerator.generatePage = function(styleguide, sections, root, sectionRoots, partials) {
  ...
  if (root === 'styleguide.homepage') {
    ...
    for (key in this.config.source) {
      if (!homepageText) {
        try {
          ...
          if (files.length) homepageText = ' ' + marked(fs.readFileSync(files[0], 'utf8'));
    ...
    console.log(' - section ' + root + ' [', styleguide.section(root) ? styleguide.section(root).header() : 'Unnamed', ']');
  }
